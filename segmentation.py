...
OUTPUT_DIR  = 'output/segmentation'     # all results written here
...
# These constants control how the OpenCV detector decides what counts as an A4 sheet.
...
def undistort_frame(frame, camera_matrix, dist_coeffs):
    ...
    """
    h, w = frame.shape[:2]
    # getOptimalNewCameraMatrix returns a refined matrix that keeps all valid pixels
    ...
    # Crop to the valid region reported by the refined matrix
    x, y, w, h = roi
...
def detect_a4_sheet(frame):
    """
    Detect the A4 reference sheet in the frame using colour thresholding and contour analysis.

    Strategy:
      1. Convert to HSV — white objects have low saturation and high value.
      2. Threshold to isolate bright, near-white regions.
    ...
    # Step 2 — threshold for white: any hue, low saturation, high brightness
    ...
    # Step 4 — find all contours in the white mask
...
def main():
    print("=== Step 3 — Instance Segmentation (Product) ===\n")
